...
if __name__ == "__main__":
    ...
    # Normalize Index column in fines_df for robust matching
    ...
    # Only print the codes, comma-separated, no brackets or extra formatting
    if result.strip().lower() != "none":
        # Remove any brackets and whitespace, split by comma
        ...
        if not matched.empty:
            print("Index,Offence,Fine")
            for _, row in matched.iterrows():
                print(f"{row['Index']},{row['Offence']},{row['Fine']}")
    else:
        print("none")
