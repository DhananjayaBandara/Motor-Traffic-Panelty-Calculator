...
class MotorTrafficActRAG_BM25:
    ...
    def _process_data(self, df):
        ...
        for index, row in df.iterrows():
            c1_content = self._clean_text(row['c1']) if pd.notna(row['c1']) else ""
            c6_content = self._clean_text(row['c6']) if pd.notna(row['c6']) else ""
            searchable_text = f"{c1_content} {c1_content} {c6_content}".strip()
            if searchable_text and len(searchable_text) > 5:
                processed_data.append({
                    'index': index,
                    'searchable_text': searchable_text,
                    'c1': str(row['c1']) if pd.notna(row['c1']) else "",
                    ...
                    'c6': str(row['c6']) if pd.notna(row['c6']) else ""
                })
    ...
    def _load_or_create_bm25(self):
        if os.path.exists(self.embeddings_cache_file):
            ...
            print(f"Loaded BM25 index from {self.embeddings_cache_file}")
        else:
            ...
            print(f"Created and cached BM25 index to {self.embeddings_cache_file}")

    def _expand_query(self, query_tokens):
        """Enhanced query expansion using domain synonyms and vocabulary matching"""
        expanded_tokens = query_tokens.copy()
        for token in query_tokens:
            # Expand using domain synonyms if available
            if token in DOMAIN_SYNONYMS:
                expanded_tokens.extend([syn for syn in DOMAIN_SYNONYMS[token] if syn not in expanded_tokens])
    ...
    def find_multiple_matches(self, scenario_text, min_score_threshold=0.2, max_matches=3):
        if not scenario_text:
            return []
        tokenized_query = scenario_text.lower().split()
        scores = self.bm25.get_scores(tokenized_query)
        valid_indices = np.where(scores >= min_score_threshold)[0]
    ...
    def analyze_testdata(self, input_file="testdata.csv", output_file="analyzed_testdata.csv"):
        ...
        for index, row in tqdm(df.iterrows(), total=len(df), desc="Analyzing scenarios"):
            ...
            if match_results:
                ...
                print(f"  ✓ Matched to {len(match_results)} section(s): {', '.join(sections)} (BM25 Scores: {', '.join(scores)})")
            else:
    ...
    def search(self, query, top_k=5, min_score=0.1):
        ...
        tokenized_query = self._tokenize_advanced(query_cleaned)
    ...
    def format_results(self, results):
        ...
        formatted_output = "=== MOTOR TRAFFIC ACT - SEARCH RESULTS (Enhanced BM25) ===\n\n"
        for i, result in enumerate(filtered_results, 1):
            formatted_output += f"Result {i} (Relevance Score: {result['similarity_score']:.4f}):\n"
...
def main():
    print("Initializing Motor Traffic Act RAG System (BM25)...")
    print("=" * 60)
    try:
