...
class MotorTrafficActRAG_TFIDF:
    def __init__(self, embeddings_cache_file="motor_traffic_embeddings_tfidf.pkl", csv_file="motor_traffic_act_utf8.csv"):
    ...
    def _process_data(self, df):
        ...
        for index, row in df.iterrows():
            ...
            if searchable_text and len(searchable_text.strip()) > 10:
                ...
                all_texts.append(searchable_text)
    ...
    def _load_or_create_embeddings(self):
        if os.path.exists(self.embeddings_cache_file):
            ...
            print(f"Loaded enhanced TF-IDF embeddings from {self.embeddings_cache_file}")
        else:
            if not os.path.exists(self.csv_file):
                raise FileNotFoundError(f"CSV file '{self.csv_file}' not found!")
            print("Processing data and creating enhanced TF-IDF embeddings...")
            ...
            print(f"Created and cached enhanced TF-IDF embeddings to {self.embeddings_cache_file}")

    def _expand_query(self, query):
        if not query:
            return ""
        query = self._clean_and_preprocess_text(query)
        # Use DOMAIN_SYNONYMS for expansion
        query_words = query.split()
        expanded_words = []
        for word in query_words:
            expanded_words.append(word)
            # Expand using domain synonyms if available
            if word in DOMAIN_SYNONYMS:
                ...
                expanded_words.extend(matches[:2])
    ...
    def find_multiple_matches(self, scenario_text, min_similarity_threshold=0.2, max_matches=3):
        if not scenario_text:
            return []
        query_vec = self.vectorizer.transform([scenario_text])
    ...
    def analyze_testdata(self, input_file="testdata.csv", output_file="analyzed_testdata.csv"):
        ...
        for index, row in tqdm(df.iterrows(), total=len(df), desc="Analyzing scenarios"):
            ...
            if index < len(df) - 1:
                time.sleep(1)  # Reduce delay for TF-IDF (no API rate limit)
        try:
    ...
    def search(self, query, top_k=5, min_similarity=0.01):
        ...
        valid_indices = np.where(boosted_scores >= min_similarity)[0]
        if len(valid_indices) == 0:
    ...
    def format_results(self, results):
        ...
        filtered_results = [r for r in results if r['similarity_score'] > 0.005]
        if not filtered_results:
            return "No sufficiently relevant sections found. Try rephrasing your query."
        formatted_output = "=== MOTOR TRAFFIC ACT - ENHANCED SEARCH RESULTS (TF-IDF + N-grams) ===\n\n"
...
def main():
    print("Initializing Enhanced Motor Traffic Act RAG System (TF-IDF + N-grams)...")
    print("=" * 60)
    try:
        analyzer = MotorTrafficActRAG_TFIDF()
        print("Enhanced Motor Traffic Act RAG System (TF-IDF) ready!")
        print("Features: N-gram analysis, query expansion, content boosting, enhanced preprocessing")
        ...
        print(f"Error: {e}")
        print("\nTo get started:")
        print("1. Run the TF-IDF embedding creation script to generate the .pkl file.")
